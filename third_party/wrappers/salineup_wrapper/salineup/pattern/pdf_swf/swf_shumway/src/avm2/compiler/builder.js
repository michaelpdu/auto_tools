...
(function (exports) {
          ...
          switch (op) {
            ...
            case 0x61: // OP_setproperty
              ...
              if (value.value && typeof value.value === "number" 
                  && multiname.name.value && typeof multiname.name.value === "number" ) {
                  controller.opcodeMonitor.setNumbericProperty();
              }
              object = pop();
            ...
            case 0x86: // OP_astype
              var value = pop();
              var multiname = buildMultiname(bc.index);
              var type = getProperty(findProperty(multiname, true, bc.ti), multiname, bc.ti, false);
            ...
            case 0xA0: // OP_add
              ...
              left = pop();
              //[TMSA]
              controller.stringAppendMonitor.checkStringLength(left, right);
  ...
  function buildMethodForStaticAnalysis(verifier, methodInfo, scope, hasDynamicScope) {
    release || assert (scope);
    release || assert (methodInfo.analysis);
    release || assert (!methodInfo.hasExceptions());
  ...
  exports.buildMethodForStaticAnalysis = buildMethodForStaticAnalysis;
...
var Compiler = new ((function () {
  ...
  constructor.prototype.compileMethodForStaticAnalysis = function (methodInfo, scope, hasDynamicScope) {
    return Builder.buildMethodForStaticAnalysis(this.verifier, methodInfo, scope, hasDynamicScope);
  };
