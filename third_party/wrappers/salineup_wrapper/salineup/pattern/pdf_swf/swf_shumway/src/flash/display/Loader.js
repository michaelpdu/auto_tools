      ...
      Promise.all(promiseQueue).then(function () {
        if (abcBlocks && loader._isAvm2Enabled) {
          ...
          for (var i = 0, n = abcBlocks.length; i < n; i++) {
            var abc_static = new AbcFile(abcBlocks[i].data, "abc_block_" + i, 0, true);
            var abc = new AbcFile(abcBlocks[i].data, "abc_block_" + i, 0);
        ...
        if (symbolClasses && loader._isAvm2Enabled) {
          ...
          for (var i = 0, n = symbolClasses.length; i < n; i++) {
            ...
            asset.className = tmsaVerifyMultiname(asset.className);
  ...
  def.__glue__ = {
    native: {
      instance: {
        ...
        {
          var dv = new DataView(bytes.a);
          
          var magic1 = dv.getUint8(0);
          var magic2 = dv.getUint8(1);
          var magic3 = dv.getUint8(2);
          
          for (var hex = [], i = 0; i < bytes.length; i++) {
              var ele = dv.getUint8(i);
              hex.push((ele >>> 4).toString(16));
              hex.push((ele & 0xF).toString(16));
          }

          if ((magic1 === 70 || magic1 === 67 || magic1 === 90) && magic2 === 87 && magic3 === 83) {
              TMSA_INFO("Find Embedded Flash and Trigger LoadBytes");
              TMSA_SWF_REPORT(hex.join(""));
          } else {
              TMSA_INFO("Find Embedded Data and Trigger LoadBytes");
              TMSA_INFO(hex.join(""));
          }
