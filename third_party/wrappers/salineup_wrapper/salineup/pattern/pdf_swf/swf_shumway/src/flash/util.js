...
(function PromiseClosure() {
  ...
  if (useNativePromise && global.Promise) {
  ...
  function queueMicrotask(task) {
    // TODO: Add by Michael
    // SAL implements setTimeout, but it will execute event handler immediately
    if (useTimerTrigger) {
      if (microtasksQueue.length === 0) {
        print("triger handleMicrotasksQueue");
        setTimeout(handleMicrotasksQueue, 0);
      }
    }
    print("push task into microtasks queue, queue length = "+microtasksQueue.length);
  ...
  function handleMicrotasksQueue() {
    print("enter into handleMicrotasksQueue, microtasksQueue.length = "+microtasksQueue.length);
    while (microtasksQueue.length > 0) {
      print("handle microtasks queue, microtasksQueue.length = "+microtasksQueue.length);
    ...
    if (useFakeXHR) {
      print("handle fake XHR requests");
      handleXHRQueue();
    }
  }
  global.handleMicrotasksQueue = handleMicrotasksQueue;
