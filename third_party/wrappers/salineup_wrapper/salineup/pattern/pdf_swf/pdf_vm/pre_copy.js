...
// AddXfaField
function AddXfaField( name, value, w, h, x ) {
    var fieldNode = new XfaFieldNode( name, value, w, h, x );
    xfa[name] = fieldNode;
    this[name] = fieldNode;
    fieldList.push(this[name]);
}

function XfaFieldNode(name, value, w, h, x) {
    this.name = name;
...
XFAObject.prototype.toString = function() {
    return "["+this.__xml.name+" XFAObject]";
}
...
function __initXFAGlobalObjects(xfaObj, xfaGlobal) {
    if (!!xfaObj.name && (xfaObj.__xml.name == "field" || xfaObj.__xml.name == "draw" || xfaObj.__xml.name == "text")) {
