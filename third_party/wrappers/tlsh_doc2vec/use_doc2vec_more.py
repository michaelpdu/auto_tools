...
class UseDoc2vec:
    def __init__(self):
        ...
        self.model_path = "/sa/middle_dir/test_only"
    ...
    def train_doc2vec_model(self):
        ...
        doc2vec_model = os.path.join(self.doc2vec_model_path,"doc2vec_{}.model".format( time.strftime('%Y%m%d%H%M%S', time.localtime())))
    ...
    def train_and_score(self):
        ...
        print '[Change Working Dir] ' + working_dir
        os.chdir(working_dir)
        cmd = 'python train.py {}'.format(self.train_file)
        os.system(cmd)
        print cmd

        train_file_log = os.path.join(self.train_file_path,"train_log.txt")
        cmd = 'python predict.py {} > {}'.format(self.train_file,train_file_log)
        os.system(cmd)
        print cmd

        test_file_log = os.path.join(self.test_file_path,"test_log.txt")
        cmd = 'python predict.py {} > {}'.format(self.test_file,test_file_log)
        os.system(cmd)
        print cmd
        ...
        for validation_file in self.validation_feature_file_set:
            try:
                ...
                cmd = 'python predict.py {} > {}'.format(validation_file,log_file)
                os.system(cmd)
                print cmd
            except:
                print "failed"
