...
TM_SA_HTTP_SPEC_PROCESS_ID              = 8
TM_SA_HTTP_SPEC_THREAD_ID               = 9
# following content type are used in local script
TM_SA_HTTP_SPEC_PROCESS_NAME            = 10
TM_SA_HTTP_SPEC_FILE_PATH               = 11
TM_SA_HTTP_SPEC_CMD_LINE                = 12
TM_SA_HTTP_SPEC_PROC_CHAIN              = 13
TM_SA_HTTP_SPEC_SHA1                    = 14
TM_SA_HTTP_SPEC_HOST_TYPE               = 15
...
TMSA_LOGLEVEL = {
    'off':TM_SA_LOG_OFF,
    'fatal':TM_SA_LOG_FATAL,
    'error':TM_SA_LOG_ERROR,
    'warn':TM_SA_LOG_WARN,
    'info':TM_SA_LOG_INFO,
    'debug':TM_SA_LOG_DEBUG,
    'trace':TM_SA_LOG_TRACE,
    'all':TM_SA_LOG_ALL,
}
...
# load library
python_version_bit = re.search(r'(\d+)\sbit',sys.version).group(1)
if python_version_bit == '32':
    tmsa = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'tmsa32.dll')
    salhelperdll = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'SALHelper32.dll')
else:
    tmsa = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'tmsa64.dll')
    salhelperdll = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'SALHelper64.dll')
tmsadll = cdll.LoadLibrary(tmsa)
salhdll = cdll.LoadLibrary(salhelperdll)
...
# alias
SALHelper_Pack = salhdll.SALHelper_Pack
SALHelper_Unpack = salhdll.SALHelper_Unpack
SALHelper_EncryptAndDecryptPattern = salhdll.SALHelper_EncryptAndDecryptPattern
...
def _define_prototypes():
    prototypes = [
        ...
        ( SALHelper_Pack,                               None,     [c_char_p, c_char_p] ),
        ( SALHelper_Unpack,                             None,     [c_char_p, c_char_p] ),
        ( SALHelper_EncryptAndDecryptPattern,           None,     [c_char_p, c_wchar_p, c_wchar_p] ),
    ]
