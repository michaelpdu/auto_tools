...
class Handlers
{
    ...
    */
    ...
    RulesStringValue(17,"&Firefox 33", "Mozilla/5.0 (Windows NT 6.3; WOW64; rv:33.0) Gecko/20100101 Firefox/33.0")
    ...
    RulesStringValue(20,"Chrome", "Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.62 Safari/537.36")
...
*/
    ...
    static var LOG = FiddlerApplication.Log;
    ...
    case "import":
        if (sParams.Length != 2) {
            LOG.LogString("Usage: import *.pcap");
            return false;
        }
        LOG.LogFormat("Import pcap file: {0}", sParams[1]);
        // import pcap file
        var oImportOptions = FiddlerObject.createDictionary(); 
        oImportOptions.Add("Filename", sParams[1]); 
        FiddlerApplication.DoImport("Packet Capture", true, oImportOptions, null);
        LOG.LogFormat("Import pcap file successfully.");
        FiddlerObject.StatusText = "Import pcap file successfully.";
        return true;
    case "savesaz":
        if (sParams.Length != 2) {
            LOG.LogString("Usage: savesaz *.saz");
            return false;
        }
        LOG.LogFormat("Save sessions to saz file: {0}", sParams[1]);
        // save to saz file
        FiddlerApplication.UI.actSelectAll();
        FiddlerApplication.UI.actSaveSessionsToZip(sParams[1]);
        FiddlerApplication.UI.actRemoveAllSessions();
        LOG.LogFormat("Save sessions to saz file successfully.");
        FiddlerObject.StatusText = "Save sessions to saz file successfully.";
        return true;
    case "clear":
        if (sParams.Length != 1) {
            LOG.LogString("Usage: clear");
            return false;
        }
        FiddlerApplication.UI.actRemoveAllSessions();
        LOG.LogFormat("Clear all of sessions successfully.");
        FiddlerObject.StatusText = "Clear all of sessions successfully.";
        return true;
    default:
