...
src = None
dst = None
ethsrc = "00:0c:29:11:11:11" 
ethdst = "00:0c:29:22:22:22"
...
def build_handshake(src,dst,sport,dport):
    print "build-handshake: src=%s, dst=%s, sport=%s, dport=%s" % (src,dst,sport,dport)
    ipsrc   = src
    ipdst   = dst
    portsrc = sport
    portdst = dport
...
#    server_isn = random.randint(1024, (2**32)-1)
    ...
    syn = Ether(src=ethsrc, dst=ethdst)/IP(src=ipsrc, dst=ipdst)/TCP(flags="S", sport=portsrc, dport=portdst, seq=client_isn)
    synack = Ether(src=ethdst,dst=ethsrc)/IP(src=ipdst, dst=ipsrc)/TCP(flags="SA", sport=portdst, dport=portsrc, seq=server_isn, ack=syn.seq+1)
    ack = Ether(src=ethsrc,dst=ethdst)/IP(src=ipsrc, dst=ipdst)/TCP(flags="A", sport=portsrc, dport=portdst, seq=syn.seq+1, ack=synack.seq+1)
    pktdump.write(syn)
...
def build_finshake(src,dst,sport,dport,seq,ack):
    print "build-finshake: src=%s, dst=%s, sport=%s, dport=%s, seq=%s, ack=%s" % (src,dst,sport,dport,seq,ack)
    ipsrc   = src
    ipdst   = dst
    portsrc = sport
    portdst = dport
    finAck = Ether(src=ethsrc,dst=ethdst)/IP(src=ipsrc, dst=ipdst)/TCP(flags="FA", sport=sport, dport=dport, seq=seq, ack=ack)
    finalAck = Ether(src=ethdst,dst=ethsrc)/IP(src=ipdst, dst=ipsrc)/TCP(flags="FA", sport=dport, dport=sport, seq=finAck.ack, ack=finAck.seq+1)
...
def make_poop(src,dst,sport,dport,seq,ack,payload):
    print "make-poop: src=%s, dst=%s, sport=%s, dport=%s, seq=%s, ack=%s, len-payload:%d" % (src,dst,sport,dport,seq,ack,len(payload))
    segments = [] 
    if len(payload) > 1460:
    ...
    for segment in segments:
        p = Ether(src=ethsrc,dst=ethdst)/IP(src=ipsrc, dst=ipdst)/TCP(flags="PA", sport=sport, dport=dport, seq=seq, ack=ack)/segment
        returnAck = Ether(src=ethdst,dst=ethsrc)/IP(src=ipdst, dst=ipsrc)/TCP(flags="A", sport=dport, dport=sport, seq=p.ack, ack=(p.seq + len(p[Raw])))
        seq = returnAck.ack
        ack = returnAck.seq
        pktdump.write(p)
...
print "fiddler-raw-dir: %s" % options.fiddler_raw_dir
