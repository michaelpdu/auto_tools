...
def SALToolCheck(inputfilepath,SALToolpath):
    ...
    SALPATH="SALineup " +r"--sie-only=true"+ "  " +'"'+ inputfilepath +'"'+">" +r'sal.log'
    print SALPATH
    #print "**************open SALineup*******************"
...
def autocheck(inputrawfilepath,SIEToolpath,SALToolpath):
    ...
    for InputEK in os.listdir(inputrawfilepath):
        ...
        if SALflag==True:
            print '*******************Run SIE engine and SALineup********************'
            SIEToolCheck(os.path.join(inputrawfilepath,InputEK),SIEToolpath)
            time.sleep(3)
            SALToolCheck(os.path.join(inputrawfilepath,InputEK),SALToolpath)
            time.sleep(3)
            os.chdir(SIEToolpath)
            ...
            while sieline:
                sipattern=re.compile(r'>>')
                matchpattern=sipattern.findall(sieline)
                if matchpattern:
                    break
                else:
                    sieline=SIEfile.readline()
            ...
            while saline:
                ...
                #print '%%%%%%%%%%%%%%%%%%%match first line%%%%%%%%%%%%%%%%%%%%%'
                sapattern=re.compile(r'Now Processing File :')
                matchpattern=sapattern.findall(saline)
                if matchpattern:
                    break
                else:
                    saline=SALfile.readline()
            #match EKName,the aim is to identify if this EKName is suspicious or normal
            EKNamePattern1=re.compile(r'rawfile')
            str1=EKNamePattern1.split(sieline)[1]
            EKNamePattern2=re.compile(r'\\')
            EKNameStart=''.join(EKNamePattern2.split(str1)[2])
            #match salEKName
            SalEKNamePattern1=re.compile(r'rawfile')
            Salstr1=SalEKNamePattern1.split(saline)[1]
            SalEKNamePattern2=re.compile(r'\\')
            SalEKNameStart=''.join(SalEKNamePattern2.split(Salstr1)[2])
            while sieline:
                endpattern=re.compile(r'DateTime')
                endmatch =endpattern.findall(sieline)
                if endmatch:
                    print InputEK,' sample analysize complete!'
                    break
                ...
                if match1:
                    ...
                    #sie
                    str2=EKNamePattern1.split(sieline)[1]
                    EKNameNext=''.join(EKNamePattern2.split(str2)[2])
                    #sal 
                    #print '**********************',saline,'********************'
                    #print '**********************',sieline,'********************'
                    Salstr2=SalEKNamePattern1.split(saline)[1]
                    SalEKNameNext=''.join(SalEKNamePattern2.split(Salstr2)[2])
                    #if EKNamNext == SalEKNameNext :
                    saline=SALfile.readline()
                    judgedecisionpattern=re.compile('Decision')
                    ...
                    if decisionmatch:
                        ...
                        while saline:
                            ...
                            if matchpattern:
                                ...
                                saline=SALfile.readline()
                                #two problems:1.only have Now Processing File ,not decision
                                #             2.if file is big ,then no this record
                    if EKNameNext != EKNameStart:
                        if (status==True):
                            EKSusSampleNum+=1
                            EKdict[EKNameStart]='suspicious'
                            status=False
                            EKNameStart=EKNameNext
                        else:
                            EKdict[EKNameStart]='normal'
                            EKNameStart=EKNameNext
                    if SalEKNameNext != SalEKNameStart:
                        if Sastatus==True:
                            SaEKSusSampleNum+=1
                            SaEKdict[SalEKNameStart]='suspicious'
                            Sastatus=False
                            SalEKNameStart=SalEKNameNext
                        else:
                            SaEKdict[SalEKNameStart]='normal'
                            SalEKNameStart=SalEKNameNext
                if match2:
            ...
            SIEfile.close()
            SALfile.close()
            #when handle the last sample, we jump that sample, so when we handle another EK, we should add the last sample
            if (status==True):
                EKSusSampleNum+=1
                EKdict[EKNameStart]='suspicious'
                status=False
            ...
            while sieline:
                ...
                endmatch =endpattern.findall(sieline)
                if endmatch:
                    print InputEK,'this sample analysize complete!'
                    break
                pattern1=re.compile(r'>>')
                pattern2=re.compile(r'Final')
                match1=pattern1.findall(sieline)
...
def resultToExcel(EKtyp):
    ...
    for i in EKdict.keys():
        ...
        if SALflag==True:
            if EKdict[i]=='suspicious' and SaEKdict[i] <>'suspicious':
                ws.write(line,2,i)
                ws.write(line,3,SaEKdict[i],titelStyle)
                ws.write(line,0,i)
                ws.write(line,1,EKdict[i],titelStyle)
            elif SaEKdict[i]=='suspicious' and EKdict[i] <>'suspicious':
                ws.write(line,2,i)
                ws.write(line,3,SaEKdict[i],titelStyle)
                ws.write(line,0,i)
                ws.write(line,1,EKdict[i],titelStyle)
    ...
    #wb.save(EKtyp + '.xls')
    if SALflag == True:
        if len(detaillist) == len(Sadetaillist):
            for i in range(len(detaillist)):
                ws.write(line,3,Sadetaillist[i].path)
                ws.write(line,4,Sadetaillist[i].type)
                ws.write(line,0,detaillist[i].path)
                ws.write(line,1,detaillist[i].type)
                if detaillist[i].status =='suspicious' and Sadetaillist[i].status <>'suspicious':
                    ws.write(line,5,Sadetaillist[i].status,titelStyle)
                    ws.write(line,2,detaillist[i].status,titelStyle)
                elif detaillist[i].status =='malicious' and Sadetaillist[i].status <>'suspicious':
                    ws.write(line,5,Sadetaillist[i].status,titelStyle)
                    ws.write(line,2,detaillist[i].status,titelStyle)
                elif Sadetaillist[i].status =='suspicious' and detaillist[i].status <>'suspicious' and Sadetaillist[i].status =='suspicious' and detaillist[i].status <>'malicious':
                    ws.write(line,5,Sadetaillist[i].status,titelStyle)
                    ws.write(line,2,detaillist[i].status,titelStyle)
                #elif Sadetaillist[i].status =='suspicious' and detaillist[i].status <>'malicious':
                    #ws.write(line,5,Sadetaillist[i].status,titelStyle)
                    #ws.write(line,2,detaillist[i].status,titelStyle)
                else:
                    ws.write(line,5,Sadetaillist[i].status)
                    ws.write(line,2,detaillist[i].status)
                line +=1
                    
        else:
            print 'sie checked result don\'t match salineup checked result'
            exit(0)
    else:
        for i in range(len(detaillist)):
            ws.write(line,0,detaillist[i].path)
...
if __name__ == '__main__':
    ...
    if len(sys.argv)<4:
        print 'warning:'
        print 'argv[1]:rawfile path         argv[2]: SIETool path        argv[3]: SALTool path'
        print 'argv[1] file name must be "rawfile"'
    if len(sys.argv)==4:
        ...
        if os.path.exists(tpath):
            ...
            try:
                ...
                try:
                    ...
                    print '*****************Excel has create in ',sys.argv[2],'******************'
                    time.sleep(3)
                    try:
